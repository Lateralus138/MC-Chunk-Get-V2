...
class MainGui(QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        for widg in self.widget_object:
            try:
                self.widget[widg] = self.widget_object[widg]['Type']
            except:
                pass
            try:
                self.widget[widg].setText(self.widget_object[widg]['Text'])
            except:
                pass
            try:
                ...
                pass
            try:
                self.widget[widg].setWordWrap(
                    self.widget_object[widg]['WordWrap'])
            except:
                pass
            try:
                self.widget[widg].setAlignment(
                    self.widget_object[widg]['Align'])
            except:
                pass
            try:
                self.widget[widg].setStyleSheet(
                    self.widget_object[widg]['Style'])
            except:
                pass
            try:
                self.widget[widg].setValidator(
                    self.widget_object[widg]['Validator'])
            except:
                pass
            try:
                self.widget[widg].clicked.connect(
                    self.widget_object[widg]['Connect'])
            except:
                pass
            try:
                self.widget[widg].setToolTip(
                    self.widget_object[widg]['ToolTip'])
            except:
                pass
    ...
    def keyReleaseEvent(self, QKeyEvent):
        ...
        if numX:
            self.widget['LabelX2'].setText(numX)
        else:
            self.widget['LabelX2'].setText('')
        if numZ:
            self.widget['LabelZ2'].setText(numZ)
        else:
            self.widget['LabelZ2'].setText('')
        if numX and numZ:
            ...
            self.widget['LabelResult'].setText('')
